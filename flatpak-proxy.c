      ...
      if (line_end) /* Found end of line */
        {
          ...
          if (auth_line_is_external (line_start, line_end, TRUE))
            ...
            {
              if (g_str_has_prefix ((const char *) line_start, "AUTH ") ||
                  g_str_has_prefix ((const char *) line_start, "DATA"))
                client->auth_external_pending = FALSE;
              g_byte_array_append (forward_data, line_start,
                                  next_offset - offset);
            }
                   ...
                   const char *socket_path)
{
  ...
  if (sscanf (socket_path, "fd:%d%n", &socket_fd, &n) == 1
      && socket_path[n] == '\0')
    {
      if (socket_fd < 3)
        {
          g_warning ("Invalid socket fd passed in socket-path");
          return NULL;
        }
      socket_path = NULL;
    }
  ...
  if (proxy->socket_fd < 0)
    {
      ...
      g_autoptr(GSocketAddress) address = g_unix_socket_address_new (proxy->socket_path);
    ...
    {
      g_autoptr(GSocket) socket = g_socket_new_from_fd (proxy->socket_fd, error);
      if (socket == NULL)
        return FALSE;
